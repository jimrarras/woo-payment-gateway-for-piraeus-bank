...
Tags: ecommerce, woocommerce, payment gateway, piraeus bank, greece
...
== Description ==
This plugin adds Piraeus Bank paycenter as a payment gateway for WooCommerce. A contract between you and the Bank must be previously signed. This is an independently maintained fork of the plugin by Papaki (Enartia S.A.), https://www.papaki.com, which was itself based on the plugin by emspace.gr. It is not affiliated with or endorsed by Papaki or Piraeus Bank.
...
New "Payment brands to display" setting: pick from Visa, Mastercard, Maestro, American Express, Diners Club, Google Pay and IRIS. Clear it to fall back to the Piraeus Bank logo.
...
Fixed: billing state is no longer a required checkout field. It is optional in EMV 3DS, and stores that hide the field for single-region countries could not take card payments at all.
